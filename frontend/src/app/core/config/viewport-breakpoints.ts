export const SMARTPHONE_MAX_WIDTH = 767;
export const TABLET_MIN_WIDTH = 768;
export const TABLET_MAX_WIDTH = 1024;
export const DESKTOP_MIN_WIDTH = 1025;

// Group Play keeps a dedicated compact breakpoint due to its custom layout rules.
export const GROUP_PLAY_COMPACT_MAX_WIDTH = 1199;

export function isSmartphoneWidth(width: number): boolean {
  return width <= SMARTPHONE_MAX_WIDTH;
}

export function isCompactWidth(width: number): boolean {
  return width <= GROUP_PLAY_COMPACT_MAX_WIDTH;
}

export function isAppCompactWidth(width: number): boolean {
  return width <= TABLET_MAX_WIDTH;
}
